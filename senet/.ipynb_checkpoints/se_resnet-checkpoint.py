...
class UNet(nn.Module):
    ...
    def forward(self, x):
        for i, down in enumerate(self.down_path):
            x = down(x)
            # print(f"After down[{i}]: {x.shape}, {x.min()} to {x.max()}")
            x = self.layers[i](x)  ## batch size 맞나? x.size(0)이? 확인
            # print(f"After layers[{i}]: {x.shape}, {x.min()} to {x.max()}")
...
def unet(**kwargs):
    model = UNet(CifarSEBasicBlock, 3, **kwargs)
    return model
